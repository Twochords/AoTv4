...
### ⚠️⚠️ THE AURA REWORK — eight of the sixteen were FLAT and could never scale (v38, 2026-08-08)
Reported as *"some of our auras feel really strong and some don't"*. It was not a tuning problem:
**eight were percentages and eight were fixed numbers** — damage shields of 2-3, weapon procs for 6-7
damage, a 10-point heal, 4 mana, 15 mana on a kill. Those were small at launch, are noise now, and get
worse every time gear or mob damage moves. v38 converts the eight flat ones and gives each a distinct
**role**, so a group running several auras gets a kit rather than duplicates.
📌 One aura slot per character, but auras are **group-shared**, so a six-class group can have six up at
once. That is why they are deliberately different in *kind* rather than in magnitude.

| class | effect spell | after v38 |
|---|---|---|
| Cleric | 43551 | SPA 125 `ImprovedHeal` 20 — the group's heals land 20 pct harder |
| Paladin | 43552 | SPA 111 `ResistAll` 15 (⚠️ physical resists are **not** modified by it) |
| Ranger | 43553 | SPA 216 `Accuracy` 15, limit **-1** = all skills |
| Druid | 43555 | SPA 3 `MovementSpeed` 25 + SPA 0 `CurrentHP` 5/tic |
| Bard | 43557 | SPA 189 `CurrentEndurance` 5/tic + SPA 15 `CurrentMana` 3/tic |
| Necromancer | 43560 | trigger unchanged; **43575** payload → SPA 147 `PercentalHeal` 2 pct + 25 mana |
| Magician | 43562 | SPA 218 pet crit 15 / 215 pet avoidance 10 / 397 pet mitigation 10 |
| Beastlord | 43564 | SPA 119 `AttackSpeed3` 10 — **overhaste**, see below |

The eight already-percentage auras are untouched: Warrior (162 mitigation 10 pct **uncapped** — still the
strongest thing on the list), Shadowknight (178), Monk (172), Rogue (169), Shaman (323 → 10 pct slow proc),
Wizard (294), Enchanter (127), Berserker (330).

- ⚠️⚠️ **spdat.h's `[AA]` TAG DOES NOT MEAN "AA ONLY".** `PetMaxHP`/`PetAvoidance`/`PetCriticalHit`/
  `PetMeleeMitigation` are all tagged `[AA]` and all four have cases in **`Mob::ApplySpellsBonuses`**, so
  they work fine from a spell. The inverse trap is already recorded in `zone/aotv4_tank_aa.cpp`
  (`ApplyAABonuses` has **no** case for `MitigateMeleeDamage`). **Check the function you actually need;
  the header comment is descriptive of live usage, not a restriction.**
- ⚠️⚠️ **`PetMaxHP` (213) IS DELIBERATELY UNUSED.** `Mob::MakePet` reads it at **summon time**
  (`zone/pets.cpp:140`), so it does nothing for a pet you already have and only applies if the aura was
  up when you summoned. The other three are read **live** off the owner in combat (`attack.cpp:320`,
  `:5794`), which is why the Magician aura uses those.
- ⚠️ **`formula` 100 / `max` 0 on every slot.** §5 records that a level-scaled formula keys off the
  **caster's** level — and the caster here is the **aura NPC**, not the player, so a scaling formula
  would read the wrong level entirely.
- ⚠️ **A focus effect takes the HIGHEST value, not the sum** (`spell_effects.cpp:5248`), so two Clerics
  in a group is 20 pct, not 40. Stock focus behaviour, not a bug.
- ⚠️ Numbers are calibrated against **stock rows read out of the DB**: endurance regen 5 is
  *Aura of the Chameleon Effect*, itself a stock aura (stock duration buffs run 2/3/5); accuracy 15 is
  stock *Accuracy I* (the line is 15/30/45); movement 25 is about half a SoW. 📌 The three **pet** values
  have **no stock spell precedent at all** (AA-only on live) and are the most likely to need a second pass.
- 📌 43570 *Reprisal* is now **orphaned** (the Cleric aura no longer procs it); 43573 *Spirit Chill* is
  untouched and still used by the Shaman aura, which was already a percentage.

#### ⚠️⚠️ OVERHASTE IS SPA **119**, NOT 98 — and 98 is silently dead at our level cap
`Client::CalcHaste` (`zone/client_mods.cpp:1013-1056`) applies the main haste cap and **then** adds
`hastetype3`, so **119 `AttackSpeed3` is the only effect that goes past the cap** — that is what
"overhaste" means here.
- ⚠️⚠️ **SPA 98 `AttackSpeed2` is gated on `level > 49`.** At our cap of **30** (`era_system.M.HARD_CAP`)
  it contributes **zero** — it applies, shows in the buff window, and does nothing. It is also the one
  that *looks* like the obvious haste-2 choice (it is the Melody of Ervaj effect).
- ⚠️⚠️ **The two read their base differently**: 98 is `effect_value - 100` (110 = +10 pct); **119 is used
  directly** (10 = +10 pct). Writing 110 on a 119 row asks for +110 pct.
- ⚠️ **At level ≤ 50 the v3 contribution is HARDCODED to a maximum of 10.** `Character:Hastev3Cap` (25)
  only applies at 51+. So 10 is the **engine ceiling**, not a tuning choice. To exceed it: either
  `Character:IgnoreLevelBasedHasteCaps = true` — ⚠️ which also removes the main `level + 25` haste cap
  **and** the item haste cap, a server-wide melee change rather than a knob — or a one-line change so the
  1-50 branch reads the rule instead of the literal `10`.
- 📌 Worth having regardless: of **168** spells carrying SPA 119, **zero** are learnable at level 30 or
  below, so this aura is the **only source of over-cap attack speed on the server**.
- 📌 Main haste cap at level 30 is `level + 25` = **55 pct**, so the aura takes a capped group from 155 to
  165 — about **6.5 pct more swings**.
...
## 41. ⚠️⚠️ THE GM FLAG HID FOUR SEPARATE BUGS IN ONE SESSION — 2026-08-07/08

Not a feature. A pattern, and the most expensive one this project has: **testing with the GM flag on
silently bypasses at least four independent gates**, so a bug can be live for every player while being
completely invisible to whoever is checking. Each of these presented as "it works for me":

| Gate | Bypass | What it hid |
|---|---|---|
| `Client::ZonePC` expansion check | `!GetGM()` (`zone/zoning.cpp:367`) | the LDoN delve half was unenterable (§24, v37) |
| `Client::CheckFizzle` | *"Your GM flag prevents you from fizzling"* | every non-GM fizzles constantly at 1-10 casting skill |
| trap trigger | `!cur->GetGM()` (`zone/trap.cpp:316`) | delve traps firing on players (§24, v39) |
| region lock | GM exempt | the same delve maps, one layer up (v27) |

📌 **Test as a non-GM before believing anything is fixed.** Where a GM character must be used, say so
and say what it cannot prove — the Anthem de Arms test was valid *because* fizzle was not the question.

## 42. The threat rework — shields, and every other special — 2026-08-08

Reported as "we need to remove the threat generation on all combat abilities besides slam and bash",
then refined to encouraging shield tanks. Five rules, all in `common/ruletypes.h`, all live-tunable
with `#reloadrules`:

| Rule | Default | Effect |
|---|---|---|
| `AoT:SpecialBonusThreat` | 1 | bonus threat every special EXCEPT Bash adds (was: base damage) |
| `AoT:BashThreatMultiplier` | 2 | Bash threat doubled, **after** shield AC and Furious Bash |
| `AoT:ShieldMeleeHatePct` | 50 | +50% threat on every swing **and** every spell while shielded |
| `AoT:BashShieldDamagePct` | 50 | +50% Bash damage with an actual shield (not a 2H) |
| `AoT:BashACCapMultiplier` | 3 | widens the cap on how much shield AC reaches Bash damage |

Plus `Combat:BashACBonusDivisor` 25 → **2** (migration v44).

- ⚠️⚠️ **"SLAM" AND "BASH" ARE THE SAME SKILL** — Slam is Bash with no shield equipped
  (`// SLAM - Bash without a shield equipped`, `Client::OPCombatAbility`). The exclusion is one skill
  id; looking for a separate Slam skill finds nothing.
- ⚠️⚠️ **A BARE SLAM DOES NOTHING, IN STOCK.** Its branch checks the secondary AND shoulder slots for
  weapon damage; with neither it sets `damage = -5` and leaves `hate_override` at **0**.
  `GetBaseSkillDamage(SkillBash)` agrees independently — it `return 0`s with no shield and no
  two-hander. So no damage and no threat, and no multiplier can rescue it.
- ⚠️ Specials keep **damage-derived** hate. Only the second, additive packet is capped — a special
  producing literally zero threat would mean hitting something without it noticing.
- 📌 **Taunt needs no exception**: no damage path, never reaches `DoSpecialAttackDamage` (§19).

### ⚠️⚠️ THE INHERITED "Shield hate boost" COMMIT WAS NEARLY A NO-OP — BOTH HALVES
Merged from `135c03b4c` and corrected. Both faults are the kind that look right in review:
1. **The melee half was inside `if (opts)`.** `Mob::Attack` takes `ExtraAttackOptions *opts = nullptr`
   (`zone/mob.h:241`), so an ordinary auto-attack passes none and that block never runs — which was
   precisely the case it was written for. Moved outside.
   📌 The arithmetic was fine: `hate_percent` is a `float(1.0f)`, so `1.0f * 150 / 100` promotes to
   1.5 with no integer truncation. It worked; it just never ran.
2. **The spell half multiplied `non_modified_aggro` alone**, which is assigned only by
   `SpellEffect::ReduceHate`/`InstantHate` (`zone/aggro.cpp:1501`) and is **0** for every other spell.
   C++ precedence made it read as though the whole sum were scaled. Now applied to the finished total.

### ⚠️⚠️ AC CANNOT ENCOURAGE SHIELDS HERE — THE ITEMS ARE TOO SMALL
Measured against our own data: shields droppable below level 30 average **6.6 AC** (80 items) and
merchant shields **6.3** (62 items); best base is 93. At 6 AC every AC-scaled bonus is invisible once
`static_cast<int>(ac_bonus + skill_bonus)` truncates. So AC supplies the **gradient** (a better shield
is better) and `BashShieldDamagePct` supplies the **floor** (any shield is worth carrying). Neither
works alone.
- ⚠️⚠️ **THE CAP, NOT THE DIVISOR, WAS THE BINDING LIMIT.** Stock caps AC's contribution at exactly
  `skill/10`, so at 300 skill AC could never add more than 30 however good the shield — and at divisor
  25 you needed **750 AC on one shield** to reach even that. Retuning the divisor alone does nothing.
- ⚠️⚠️ **`reclevel` IS USELESS FOR "WHAT CAN A PLAYER GET"** and it produced a wrong answer here
  (a "74 AC average" that sent the first tuning pass astray). §26 already records this; classic gear
