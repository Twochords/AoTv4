...
// ⚠️⚠️ THE DROPDOWN IS HIDDEN UNLESS THE UNIVERSAL KIT IS OPEN. A native station links exactly ONE
// tradeskill, so filtering there can only ever return nothing -- picking Blacksmithing at a baking
// table looked broken because the request was genuinely empty. The Kit links 13 tradeskills, which is
// the only place a filter has anything to do.
// ⚠️ The dll cannot work this out for itself: `CTradeskillWnd` is not in EQClasses.h and nothing
// tradeskill-related is mapped in eqgame.h, so there is no accessor for the open container -- and
// reading a raw struct offset is the documented crash (section 13). The SERVER knows the container
// (`RecipesSearch_Struct::some_id`) and says so with "TSKIT <0|1>" on every search.
// 📌 A HINT, not the rule. The server ignores the skill token outside the Kit regardless, so
// un-hiding this in a modified client gains nothing.
static bool  g_tsKitOpen    = false;
...
void TS_DriveDropdown()
{
	...
	// ⚠️ HIDDEN, not disabled: this build maps no reliable enable setter (EQClasses.h only DECLARES
	// CXWnd::IsEnabled, and writing a raw member offset is section 13's crash). `Show` is mapped.
	// Same call section 16 makes for the Advanced Loot buttons, for the same reason.
	((CXWnd*)combo)->Show(g_tsKitOpen ? 1 : 0, 1);
	if (!g_tsKitOpen) {
		// ⚠️ Reset the remembered choice while hidden, or reopening the Kit fires a stale filter for
		// a skill the player picked at some other station.
		g_tsLastChoice = 0;
		return;
	}
...
// "TSKIT <0|1>" -- is the open tradeskill container our Universal Kit? Called from the dsp_chat
// swallow chain; returns true so the line never reaches the player's chat log.
bool TradeskillParseTransport(const char* message)
{
	if (!message) { return false; }
	const char* t = strstr(message, "TSKIT ");
	if (!t) { return false; }
	g_tsKitOpen = (atoi(t + 6) != 0);
	return true;
}
