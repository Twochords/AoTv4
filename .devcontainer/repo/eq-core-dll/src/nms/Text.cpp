...
// AoTv4: one sprite shared by every floating number, owned by FloatingTextManager.
// ⚠⚠ UPSTREAM CREATED AN ID3DXSprite PER DAMAGE NUMBER, AND THAT IS A LEADING SUSPECT FOR
// "the numbers show up sometimes". D3DXCreateSprite is a device resource allocation; on a busy frame it
// can fail, and when it does Initialize returns false, _uiSprite stays null, and Render early-outs --
// so the hit is silently skipped with nothing logged and no visible difference from a missed packet.
// A shared sprite removes the per-hit allocation entirely, so the only way to have no sprite is to have
// no device.
extern ID3DXSprite* g_pFtSharedSprite;
...
Text::~Text()
{
	// ⚠⚠ NULL CHECK, AND OWNERSHIP. Upstream released unconditionally, so a Text whose
	// D3DXCreateSprite had failed crashed the client on destruction -- i.e. the failure path above
	// turned an invisible number into a crash on the next Cleanup. Releasing the SHARED sprite would
	// be worse still: it would tear the sprite out from under every other number on screen.
	if (_uiSprite && _ownsSprite) { _uiSprite->Release(); }
	_uiSprite = nullptr;
}

bool Text::Initialize(ID3DXFont* fontPtr, std::string text)
{
	if (g_pFtSharedSprite)
	{
		_uiSprite = g_pFtSharedSprite;
		_ownsSprite = false;
	}
	else if (SUCCEEDED(D3DXCreateSprite(g_pDevice, &_uiSprite)))
	{
		_ownsSprite = true;
	}
	else
