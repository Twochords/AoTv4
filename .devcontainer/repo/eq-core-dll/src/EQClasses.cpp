...
#endif
// AoTv4: LoadIniInfo / StoreIniInfo were DECLARED in EQClasses.h and their offsets were both defined
// (eqgame.h) and initialised (MQ2Globals.cpp INITIALIZE_EQGAME_OFFSET) -- but no thunk was ever
// emitted for them here, so any caller failed at LINK time with an unresolved external. That is a
// third state beyond CLAUDE.md section 13's "declared but not mapped": everything except the body.
// ⚠️ Checking eqgame.h for the _x offset is NOT sufficient evidence that a method is callable. The
// offset existing only means somebody recorded the address; the FUNCTION_AT_ADDRESS block below is
// what turns it into a function. Grep EQClasses.cpp for the method name before relying on one.
// 📌 Needed by core_aotmenu.cpp so the AoT bar can restore its own saved position and minimised
// state -- the client persists both under `Minimized`/`XPos`/`YPos` in UI_<char>_<server>.ini, and
// nothing in this dll was ever reading them back.
