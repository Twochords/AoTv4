...
-- in the Refining Crucible (item 147510, zone/tradeskills.cpp -- moved from 2000060 by migration v53,
-- because an id at or above 0x100000 cannot be linked in chat).
--
...
-- ---------------------------------------------------------------- Onslaught clock
-- ⚠️ MUST MATCH `tasks.duration` on the Onslaught task rows, which is 1800.
-- ⚠️⚠️ THOSE ROWS ARE **2000460-2000498**, NOT 2000340-2000345 -- this comment named the old band for
-- long enough to matter. v36 rebuilt the tasks as 39 dungeons x 6 modes and widened the mode offsets
-- from 10 to 40, so 2000340+ is now **Hard**; anyone following the old note would have retuned the
-- wrong mode. Derive it rather than trusting a literal: base 2000300 + this mode's `taskoff` (160).
-- ⚠️⚠️ AND THE TWO DID DRIFT, EXACTLY AS WARNED BELOW -- v36 wrote 21600 over every mode's duration,
-- so the journal advertised SIX HOURS against a clock that still ended the run at thirty minutes.
-- Repaired by migration v51, which matches on the title rather than on an id band precisely because
-- these offsets have already moved once.
-- The task system owns the actual failure; this constant only drives the countdown announcements, so
-- if the two drift the warnings simply lie about a deadline the engine still enforces.
...
function M.stock_chest(c, chest, run, L)
    ...
    -- ⚠️⚠️ A MISSING CHEST MUST NOT COST THE PLAYER THE AUGMENT. This used to bail out here, which
    -- turned any failure to place or resolve the chest into a SILENTLY SKIPPED REWARD -- and the
    -- caller then printed "A heavy, glowing chest settles where you struck the last blow" regardless,
    -- so the player was sent hunting for a chest that was never there. Reported from play as a boss
    -- dying with no chest ("first time out of countless boss kills, Deepest Guk: Accursed Sanctuary"),
    -- which is exactly the shape of a rare spawn failure: nothing errors and nothing is logged.
    -- 📌 This is the same conclusion the COIN already reached a few lines below -- it is paid straight
    -- to the player precisely because a reward that exists only on a corpse can be lost. The chest is
    -- presentation; the augment is the reward, and it now arrives either way.
    local npc = chest and as_npc(chest) or nil
...
-- 📌 6.0 -> 5.0 (2026-08-09): reported from play as solo wardens having "a bit much health", alongside
-- the damage retune below. A smaller step than the previous two on purpose -- health was the lesser
-- half of that report, and this number has now moved three times.
M.BOSS_HP_MULT  = 5.0

-- 📌 1.6 -> 1.25 (2026-08-09): solo wardens hitting too hard in melee. Note this multiplies damage the
-- boss ALREADY has at `eff + BOSS_LVL_BONUS`, so the level bonus is compounding with it -- 1.25 of a
-- +2-level mob is still comfortably above an ordinary one, which is the intent.
M.BOSS_DMG_MULT = 1.25
M.BOSS_LVL_BONUS = 2

-- ⚠️⚠️ THE WARDEN'S SPELL OUTPUT NEEDS ITS OWN NUMBER, BECAUSE THE SCALER CANNOT TOUCH IT.
-- `scale_spell_output` scales spell damage by `eff_level / native_level`, clamped to 1.0 so a mob is
-- never made to cast HARDER than it was authored to. The warden template (npc 2000301) is **level 1**,
-- so that ratio is always >= 1 and always clamps -- meaning the warden keeps its authored spellscale
-- at EVERY rung, and is the one mob in the delve whose casting the scaler never adjusts.
-- That was invisible while the template's 50 was simply inherited (50 is what the stock DoN mobs ship
-- with, so it read as normal). It matters now because the warden is handed a full class spell list at
-- `blvl` (2026-08-04) rather than the template's empty one -- so it casts real, level-appropriate
-- spells at an unscaled 50, on top of the melee multiplier above.
-- 📌 50 -> 30 (2026-08-09): reported as solo warden damage coming from "spells and melee" -- both
-- halves are retuned together, because cutting only melee would have left casters untouched and the
-- report named them first.
-- ⚠️ Applied AFTER the class and spell list are attached, since those are what make it meaningful.
M.BOSS_SPELLSCALE = 30
...
function M.spawn_boss(c, L, run, ordinal)
    ...
    -- ⚠️⚠️ AND PIN THE SPELL OUTPUT -- LAST, because everything above is what gives the warden spells
    -- to cast in the first place. `scale.scale_npc` set a spellscale on the way in, but for THIS mob it
    -- is always the authored value: the ratio it uses is eff/native and the template is level 1, so it
    -- clamps to 1.0 at every rung (see the note on M.BOSS_SPELLSCALE). The warden is therefore the one
    -- delve mob whose casting the scaler never adjusts, and it has to be set explicitly here.
    -- ⚠️ Safe to do after the rescale sweep exists because that sweep SKIPS anything at 2000000+
    -- (the fix that stopped it flattening wardens into trash) -- so nothing overwrites this later.
    -- ⚠️ healscale is deliberately NOT touched: the caster lists include heals, and a warden that can
    -- top itself up is a fight with a mechanic rather than a damage race. It was not in the report.
    npc:ModifyNPCStat("spellscale", tostring(M.BOSS_SPELLSCALE))
...
function M.on_task_complete(e)
    ...
    -- ⚠️⚠️ STOCK IT UNCONDITIONALLY -- `M.stock_chest` handles a nil chest by summoning the augment
    -- straight to the player. Gating the call on the spawn succeeding is what made a failed spawn2
    -- cost the reward outright, with the message below still claiming a chest was there.
    if chest and chest.valid then
        chest:AddNimbusEffect(412)
    else
        -- ⚠️ Loud and located. `eq.spawn2` returning nothing is silent, and this is the ONLY record
        -- that it happened -- without the coordinates a rare failure cannot be reproduced or tied to
        -- a spot in a dungeon. The swarm spawner at the bottom of this file already reports its own
        -- failures this way; this path did not, which is why it went unnoticed for so long.
        eq.debug(string.format(
            "[delve] chest spawn2(%d) returned nothing in zone=%s inst=%s at %.1f,%.1f,%.1f",
            M.CHEST_NPC, tostring(run.zone), tostring(run.instance), c:GetX(), c:GetY(), c:GetZ()))
    end
    M.stock_chest(c, chest, run, L)

    -- ⚠️ Only claim a chest exists when one does. Say where it is, for the case where it still ends
    -- up behind something.
    if chest and chest.valid then
...
function M.handle_say(e)
    ...
    if msg == "delveexit" then
        ...
        -- directions: the run pays NOTHING on an abandon (no score, no sigil progress, no chest, no
        -- coin -- all of that is paid in on_task_complete), while what the player dodges is the
        -- ROGUELITE DEATH itself -- level back to 1, the spellbook wiped, carried gear destroyed. On
        -- this server that is the single most expensive event there is, so "Exit" was the cheapest
        -- escape from a losing fight in the game and cost only rewards that were never banked.
        -- Reported from play as "weird, it let me leave the delve during combat".
        -- ⚠️ GetAggroCount, not IsEngaged: it stays true while something chases you after you stop
        -- swinging, which is exactly when the escape is worth most. Same idiom as M.enter.
        -- ⚠️⚠️ IT GOES HERE, NOT IN M.leave. That function is the SINGLE teardown path and its other
        -- caller is the chest's close timer (M.on_close_timer) -- gating inside it would strand a
        -- player who cleared the delve and is still being chased by a straggler, with the countdown
        -- silently refusing to close their instance.
        -- 📌 Dying and camping still end the run, so this cannot trap anyone: hate drops, or they die,
        -- or they /q and M.on_disconnect tears it down.
        if (e.self:GetAggroCount() or 0) > 0 then
            e.self:Message(MT.Red, "You cannot leave the delve while something is fighting you.")
            return true
        end
