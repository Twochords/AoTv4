...
RULE_INT(Character, GreenModifier, 5, "The experience obtained for green con mobs is multiplied by value/100")
RULE_INT(Character, LightBlueModifier, 10, "The experience obtained for light-blue con mobs is multiplied by value/100")
RULE_INT(Character, BlueModifier, 60, "The experience obtained for blue con mobs is multiplied by value/100")
RULE_INT(Character, WhiteModifier, 90, "The experience obtained for white con mobs is multiplied by value/100")
RULE_INT(Character, YellowModifier, 100, "The experience obtained for yellow con mobs is multiplied by value/100")
RULE_INT(Character, RedModifier, 120, "The experience obtained for red con mobs is multiplied by value/100")
...
RULE_INT(Spells, HealAmountMessageFilterThreshold, 0, "Heals at or below this threshold send no message. Despite the name it gates EVERY heal message, not just lifetaps -- one call site, Mob::HealDamage. 0 to Disable, which is what we run: at a level 30 cap almost every heal is small, and stock 100 hid essentially all of them. KNOWN COST of 0: a heal paid from Lua arrives with no caster and falls to the generic 'You have been healed for N points' branch, so the Thirst line (43342-43347, 2 to 12 hit points on EVERY melee hit, 3-4 times a second) prints a line per hit on top of the 3-second summary aotv4_thirst.lua already throttles itself to. Accepted deliberately. Raising this to 13 silences Thirst but also silences any real heal-over-time tick of 12 or less, which is why it is not simply set higher.")
...
RULE_BOOL(Combat, AdjustProcPerMinute, false, "Adapt the average proc rate to the speed of the weapon")
...
RULE_REAL(Combat, BaseProcChance, 0.05, "Base chance for procs")
RULE_REAL(Combat, ProcDexDivideBy, 1000, "Divisor for the probability of a proc increased by dexterity")
...
RULE_REAL(AoT, MitFloorAcCoeff,       		1.25, "AC coefficient in the mitigation floor formula: mit/(coeff*mit + scalar*off). Asymptote = 1/coeff (default 50% max floor)")
RULE_REAL(AoT, MitOffScalar,          		1.0,  "Offense multiplier in the mitigation roll denominator. Higher = offense dominates more; also raises the deflect threshold")
RULE_REAL(AoT, MitAcDR,              		0.8,  "AC coefficient in the mitigation roll denominator. Higher = more DR on stacking AC; also raises the deflect threshold")
...
RULE_REAL(AoT, MaxHitOffenseMultiplier,     0.0125,  "Determines how much player's offense is a factor in maximum melee hits. Vanilla is ~ 0.01.")
...
RULE_INT(AoT, WeaponProcIntervalNativeMs,   25000, "AoTv4: minimum time in ms between weapon procs on a NATIVE (untiered) weapon. A weapon proc is an INTERVAL here, not a per-swing chance -- the first successful hit after the interval elapses procs, then the timer restarts. 0 falls back to the stock computed chance for that tier. Tracked PER HAND, so a dual wielder gets one from each weapon on its own timer.")
RULE_INT(AoT, WeaponProcIntervalHallowedMs, 20000, "AoTv4: minimum time in ms between weapon procs on a HALLOWED weapon. See WeaponProcIntervalNativeMs.")
RULE_INT(AoT, WeaponProcIntervalMythicMs,   15000, "AoTv4: minimum time in ms between weapon procs on a MYTHIC weapon. See WeaponProcIntervalNativeMs.")
...
RULE_INT(AoT, SpecialEndurancePct,      	33,	"NO LONGER PRICES ANYTHING -- it only switches the pre-swing 'too winded' gate on and off. The endurance a damaging combat special costs is now a hardcoded level curve in Mob::DoSpecialAttackDamage (damage * 100 / (100 + level^2): about 99 percent of damage at level 1, 50 at 10, 20 at 20, 10 at 30) so that specials get cheaper with level the way a caster's mana pool outgrows a spell's cost. Set to 0 and the gate goes away but you STILL PAY; any non-zero value behaves identically. Kept as the gate's switch rather than removed. It used to be a flat percent calibrated against the Sinew line (custom/sql/aotv4_sinew_line.sql returns damage/3); the curve only passes 33 percent around level 14, so that pairing is no longer exact.")
...
RULE_INT(AoT, TradeskillFloor,          	20,	"Every tradeskill sits at least this high. Applied at CHARACTER CREATION in world (Client::SetClassStartingSkills) so a new character is born with it, and re-applied on connect and level up by floor_tradeskills in global_player.lua for existing characters. !! Both sides read THIS rule -- the Lua via eq.get_rule('AoT:TradeskillFloor') -- so the number lives in one place. A comment in global_player.lua used to claim there is no get_rule binding and duplicated the constant; there is one (lua_general.cpp, `eq.get_rule(name)` returning a string). !! Tradeskills are deliberately NOT capped like combat skills: this is a floor, it is above the level-1 cap, and section 32 records that tradeskill skill is the one thing the roguelite death does not reset.")
RULE_INT(AoT, ShieldMeleeHatePct,       	50,	"Extra threat, as a percent, on EVERYTHING a character does while a shield is equipped -- every melee swing (Mob::Attack) and every spell (Mob::CheckAggroAmount). !! The melee half MUST live outside the `if (opts)` block: Mob::Attack takes `ExtraAttackOptions *opts = nullptr` by default, so an ordinary auto-attack passes none and anything inside that block never runs on a normal swing -- which was the entire case it was written for. !! The spell half must scale the FINISHED total, not `non_modified_aggro`, which is assigned only by SpellEffect::ReduceHate/InstantHate and is 0 for every other spell. Both halves read this one rule so the number cannot drift between them. 0 disables. Bash does NOT pass through either path -- it has its own AoT:BashThreatMultiplier -- so the two never compound.")
RULE_INT(AoT, BashShieldDamagePct,      	50,	"Percent added to BASH DAMAGE when an actual SHIELD is equipped (not a two-hander). Applied to the base, the skill bonus and the AC bonus together in Mob::GetBaseSkillDamage. !! THIS EXISTS BECAUSE AC ALONE CANNOT ENCOURAGE SHIELDS ON THIS SERVER: measured against our own items, shields droppable below level 30 average 6.6 AC and merchant shields 6.3, so any AC-scaled bonus is invisible at the bottom of the curve (6/2 = 3 largely vanishes when static_cast<int>(ac_bonus + skill_bonus) truncates). AC supplies the GRADIENT -- a better shield is better -- and this supplies the FLOOR, that any shield at all is worth carrying. Neither works alone. !! It raises THREAT too, deliberately: DoSpecialAttackDamage seeds hate from base_damage before adding shield AC and AoT:BashThreatMultiplier, so a bigger Bash is a louder Bash. Set 0 to make Bash damage depend on AC alone.")
RULE_INT(AoT, BashACCapMultiplier,      	3,	"Multiplies the cap on how much shield AC may add to BASH DAMAGE, in Mob::GetBaseSkillDamage. Stock caps the AC contribution at exactly skill/10, so at 300 skill AC can never add more than 30 however good the shield is -- and with Combat:BashACBonusDivisor at 25 you would need 750 AC on one shield to reach even that. Measured on our own items (shields usable at 30: average 74 AC) the AC term was worth about 3 damage, which is why lowering the divisor alone does nothing: the CAP is the binding limit. 2 lets AC contribute up to skill/5. Set 1 for stock. !! Do not remove the cap entirely -- it is what stops a high-AC shield carrying a character who has never trained Bash.")
RULE_INT(AoT, SpecialBonusThreat,       	1,	"Bonus threat a damaging combat special adds, for every special EXCEPT Bash/Slam. Stock adds a separate hate packet in Mob::DoSpecialAttackDamage equal to the base damage, ON TOP of the hate Mob::Damage already generates from the damage dealt -- so every special was worth roughly double threat and the damage classes out-threatened the tank. Floored to a token 1 instead. !! 1, NOT 0, deliberately: AddToHateList(this, 0, 0) is the 'no hate' call used by utility paths, and 1 is the smallest value that is unambiguously still a hate contribution. !! This caps the BONUS packet only -- damage still generates hate normally, because a special producing zero threat would mean hitting something without it noticing. Set -1 to restore stock (bonus threat = base damage). See AoT:BashThreatMultiplier for the other half.")
RULE_INT(AoT, BashThreatMultiplier,     	2,	"Multiplier on Bash/Slam's threat, applied AFTER the shield AC and Furious Bash focus bonuses so those scale with it -- multiplying before would double only the base damage and halve the value of the shield bonuses. Bash is the ONE special exempt from AoT:SpecialBonusThreat, and this is what makes it the threat tool rather than merely the last one left at stock hate. !! Slam is the same skill (Bash with no shield equipped) so it is doubled too, off a base with no AC or focus contribution -- the doubling is the ability's identity, not a reward for carrying a shield. Set 1 for stock Bash threat.")
RULE_BOOL(AoT, NPCsNeverEnrage,         	true,	"Nothing enrages. Enrage makes a mob riposte EVERY frontal melee attack (zone/attack.cpp:492), turning the end of a fight into 'stop attacking and walk away'; 404 npc_types carry the ability. Checked at the top of Mob::StartEnrage, before the stock NPC:LiveLikeEnrage block and before any timer, so an enrage is never even scheduled. !! THIS EXISTS BECAUSE THE STOCK RULE WAS NOT ENOUGH. NPC:LiveLikeEnrage (set true by migration v18) works, but its header DEFAULT is false -- ENRAGE ON -- so any server that has not run v18, or whose rule_values predates it, enrages normally with nothing to indicate why. That is exactly what was reported on The Brood Mother in Velketor's while this server had the rule set correctly. Section 35 also records one content dump silently rewriting 64 rule_values rows, and a stock rule is precisely what such a dump overwrites. A code guard defaulting to the wanted behaviour survives both a missing row and a dump that clears one. NPC:LiveLikeEnrage is left alone, not reverted. Rampage is a separate ability and is untouched. Set false to restore stock enrage.")
RULE_BOOL(AoT, NPCsNeverFlee,           	true,	"Nothing flees from low health -- every creature fights to the death. Checked at the top of Mob::CheckFlee, before GetFleeRatio, so it also overrides NPCs carrying SpecialAbility::FleePercent (which is read FIRST and would otherwise ignore Combat:FleeHPRatio / FleeGrayHPRatio entirely -- a rule-only fix does NOT work for those). !! FEAR IS UNAFFECTED: a feared mob enters through StartFleeing, and the guard sits AFTER the already-fleeing speed-update block precisely so feared mobs keep moving. Move it to the top of the function and you freeze every feared mob in place and silently break fear. Set false to restore stock fleeing; SpecialAbility::FleeingImmunity and spellbonuses.ImmuneToFlee are left intact for that reason.")
...
RULE_INT(AoT, HardLevelCap,             	30,	"Hard ceiling on organic level gain, enforced at the top of Client::SetLevel. This is the AUTHORITATIVE cap -- era_system.clamp_level cannot hold one, because event_level_up fires while m_pp.level is still the old level (so its GetLevel() is stale and it misses by one) and because any SetLevel it performs is overwritten when the outer call resumes. Keep in step with era_system.M.HARD_CAP, which still gates the spell pool and pins experience at the threshold. GM #level is exempt (the `command` flag), so a GM can still exceed it to reach content. 0 disables the cap entirely.")
RULE_BOOL(AoT, SkillCapHighWater,       	false,	"SUPERSEDED and off by default -- global_player.max_skills_for_level now sets every non-tradeskill to its cap for your CURRENT level on connect and on every ding, so there is no skill progress left to preserve across a death and nothing to re-earn. Leaving this on would additionally let a level 1 character use-raise ABOVE their level's cap toward their high-water mark, which is exactly the 'keep what you had before' behaviour that rule replaced. Kept only so the older approach can be restored in one flip. Original behaviour: weapon and casting skills (1H/2H Blunt+Slash+Pierce, Hand to Hand, Archery, Throwing, Abjuration, Alteration, Channeling, Conjuration, Divination, Evocation) keep the cap of the HIGHEST level the character has ever reached, not the one their current level allows. The roguelite death sends you to level 1, where those caps are 10 -- so a hard-earned 125 Channeling could not rise again until you had re-levelled all the way back past it, and every run began by re-treading skill ground you had already won. The stored VALUE was never lost; this is purely about being allowed to keep gaining. Deliberately excludes the reward-gated combat abilities (skill_pool.lua), which death resets to 0 on purpose so they are re-earned through the level-up picker.")
...
RULE_BOOL(AoT, NPCFullHealOnReset,      	true,	"An NPC that drops combat resets to full health and mana. Stock leaves a disengaged mob on whatever health it was left at, so damage banks across attempts: chip it, run, come back, chip it again -- and the next player finds a softened mob for full loot. Applied in Mob::AI_Event_NoLongerEngaged, which every way of emptying a hate list funnels through, so it covers fleeing, feign death, the target zoning out or dying, and hate going stale. Owned mobs (pets, charmed, swarm) are excluded or a pet would full heal every time it disengaged. Set false for stock behaviour.")
RULE_BOOL(AoT, BowMinRangeIsMeleeRange, 	true,	"The minimum distance for firing a BOW becomes melee range instead of the flat Combat:MinRangedAttackDist (25). Melee range is the closest you can stand to something you are fighting, so in practice this means a bow has no minimum at all and can be fired point blank while in melee -- an archer is no longer forced to back up out of melee to shoot. Applied in Client::RangedAttack, the one choke point both player bow paths funnel through (the server-driven auto-fire loop and the manual archery combat ability). Combat:MinRangedAttackDist is left untouched so NPC archers, bots and THROWING keep the stock floor. Set false to restore the stock 25 unit minimum.")
RULE_INT(AoT, SelfBuffDurationTicks,    	43200,	"How long a beneficial buff a player casts ON THEMSELVES lasts, in ticks (6s each). 43200 = ~3 days, so re-buffing is never part of play. 0 disables the extension entirely and every buff keeps its native duration. Applied in Mob::CalcBuffDuration as a FLOOR (std::max), so a spell whose native duration is already longer is never shortened. !! THIS REPLACES THE PERMANENT-BUFF EXPERIMENT (migrations v17/v19/v20, reverted by v21) AND IT IS DELIBERATELY FINITE. Permanent means buffdurationformula 50, which makes CalcBuffDuration_formula return -1, and that -1 flows into the ramping-effect branches of CalcSpellEffectValue_formula as `ticdif = -1 - max(ticsremaining-1,0)` -- so the effect MAGNITUDE broke while the buff icon persisted (reported as 'the buff stays but the stats do not', on zoning and on death). A large finite tick count has none of that. ! It also covers a buff cast on a GROUP MATE, and Group::AoTv4FadeGroupBuffs takes those back in both directions when the group breaks up (DelMember / DisbandGroup) -- otherwise you could group for a second, blanket everybody, disband and hand three-day buffs to strangers. Anyone NOT in your group keeps the spell's native duration, which is why v21 had to put the native durations back into spells_new: you cannot hand out a native duration you have overwritten. ! BENEFICIAL bard songs are included, but only because they no longer PULSE (see the IsPulsingBardSong guard in Client::CastedSpellFinished) -- a song that both pulses and never expires locks the singer into song mode with no way to stop. Detrimental songs still pulse and are unaffected.")
RULE_CATEGORY_END()
