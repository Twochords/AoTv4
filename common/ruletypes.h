...
RULE_INT(AoT, SpecialEndurancePct,      	33,	"Endurance a damaging combat special costs, as a percent of the damage it actually dealt. 33 = damage/3, so a 200 damage Backstab costs 66 endurance (was 50, i.e. damage/2). 0 disables the cost entirely. Charged AFTER the hit, because the cost depends on the damage rolled. This now matches the Sinew line's return (custom/sql/aotv4_sinew_line.sql pays back damage/3), so one Sinew cast funds roughly its own damage worth of specials -- the two are one calibration and should move together.")
