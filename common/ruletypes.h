...
RULE_BOOL(AoT, PermanentBuffNeedsMemmed, 	true,	"A PERMANENT buff (buffdurationformula 50) fades off a player as soon as the spell that granted it is no longer sitting in one of their spell gems. This is the leash on migration v20's permanent buffs: you may keep as many permanent buffs as you have gems, and freeing a gem for something else costs you the buff. Scoped in Mob::BuffProcess to spells the player has SCRIBED, so item clicks, AA-granted and NPC-applied permanent buffs are never touched. Pairs with the rule below -- without that one, players would trade permanent buffs to each other and bypass the gem limit entirely. Set false to let permanent buffs persist regardless of what is memorised.")
RULE_BOOL(AoT, PermanentBuffSelfOnly,   	true,	"A permanent BENEFICIAL buff cast by a player on SOMEONE ELSE is not permanent -- it gets a long but finite duration instead (30 * (level + 3) ticks: 12 minutes at level 1, 99 at the level cap). Without this, the spell-gem leash above binds only a solo player: a group would buff each other and every member would hold far more permanent buffs than they have gems. Applied in Mob::CalcBuffDuration, and only when a CLIENT casts a beneficial spell on a target other than itself, so item clicks, procs, NPC buffs and self-buffs are all unaffected. Set false to let players hand each other permanent buffs.")
RULE_CATEGORY_END()
